...
var app = document.querySelector("#adminApp");
var authPanel = document.querySelector("#authPanel");
...
async function loadUsers() {
  showStatus("Felhaszn\xE1l\xF3k bet\xF6lt\xE9se...");
  const response = await fetch("/.netlify/functions/admin-users");
  const data = await response.json();
  if (!response.ok) throw new Error(data.error || "Nem siker\xFClt bet\xF6lteni a felhaszn\xE1l\xF3kat.");
...
form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    setLoggedIn(user);
    showStatus("Sikeres bel\xE9p\xE9s.", "success");
  } catch (error) {
    showStatus(error.message || "Sikertelen bel\xE9p\xE9s.", "error");
  }
});
signupForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    setLoggedIn(user);
    showStatus(user.emailVerified ? "Admin fi\xF3k l\xE9trej\xF6tt." : "Ellen\u0151rizd az email-fi\xF3kodat.", "success");
  } catch (error) {
    showStatus(error.message || "Sikertelen regisztr\xE1ci\xF3.", "error");
  }
});
inviteForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || "A megh\xEDv\xF3 k\xFCld\xE9se sikertelen.");
    inviteForm.reset();
    showStatus("A bel\xE9p\xE9si email elk\xFCldve.", "success");
    await loadUsers();
  } catch (error) {
    showStatus(error.message || "A megh\xEDv\xF3 k\xFCld\xE9se sikertelen.", "error");
  }
});
...
try {
  ...
  showStatus(error.message || "A bel\xE9p\xE9s nem \xE9rhet\u0151 el.", "error");
}
